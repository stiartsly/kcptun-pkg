#! /bin/sh
### BEGIN INIT INFO
# Provides:          shadowtun-ss
# Required-Start:    $remote_fs $syslog
# Required-Stop:     $remote_fs $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: Starts the shadowtun-ss daemon
# Description:       Starts the shadowtun-ss daemon
### END INIT INFO

# PATH should only include /usr/* if it runs after the mountnfs.sh script
PATH=/sbin:/usr/sbin:/bin:/usr/bin
DESC="ShadowTunnel for ShadowSocks"
NAME=shadowtun-ss
DAEMON=/usr/bin/shadowtun-prog
CONFIG_FILE=/etc/shadowtun/ss.conf
DAEMON_ARGS="-c $CONFIG_FILE"
PIDDIR=/var/run/shadowtun
